...
VideoIcon::VideoIcon(std::string fileName, DbConnector * dbCon):Gtk::Image()  {
  ...
  if(dbCon->video_exists(fileName)) {
    ...
    std::string cmdln1((boost::format("ffmpeg -y -nostats -loglevel 0 -ss %.03d -i %s -frames:v 1 -f image2pipe -vcodec png - | convert png:- -fuzz %i -trim -thumbnail %ix%i %s") % thumb_t % fVidFile->fixed_filename() % FUZZ % ICON_WIDTH % ICON_HEIGHT % temp_icon).str());
