...
VideoIcon::VideoIcon(std::string fileName, DbConnector * dbCon):Gtk::Image()  {
  ...
  if(dbCon->video_exists(fileName)) {
    ...
    boost::process::system((boost::format("ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 %s" ) % fileName).str().c_str(),boost::process::std_out > is);
