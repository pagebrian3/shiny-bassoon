...
VBrowser::VBrowser() {
  ...
  Gtk::Image img;
  img.set_image_from_icon_name(stock_icon,Gtk::ICON_SIZE_BUTTON);
  ...
  for (bfs::directory_entry & x : bfs::recursive_directory_iterator(path))
    {
      ...
      if(extensions.count(extension)) fFBox->add(VideoIcon(x.path().str(),dbCon));     
    }
...
int VBrowser::sort_videos(VideoIcon videoFile1, VideoIcon videoFile2) {
  ...
  if(sort_by == "size") return value*(v1.size - v2.size);
