...
VBrowser::VBrowser() {
  ...
  Gtk::Image img;
  img.set_from_icon_name(stock_icon,Gtk::ICON_SIZE_BUTTON);
  asc_button->set_image(img);
...
void VBrowser::populate_icons(bool clean) {
  ...
  //fFBox->set_sort_func(std::mem_fn(&VBrowser::sort_videos));
...
int VBrowser::sort_videos(Gtk::FlowBoxChild *videoFile1, Gtk::FlowBoxChild *videoFile2) {
  int value=1;
  vid_file v1=((VideoIcon *)videoFile1)->get_vid_file();
  vid_file v2=((VideoIcon *)videoFile2)->get_vid_file();
...
void VBrowser::fdupe_clicked(){
  ...
  for (bfs::directory_entry & x : bfs::recursive_directory_iterator(path))
	...
	vid_file vid_obj = dbCon->fetch_video(full_path.c_str());
	int video_id = vid_obj.vdatid;
...
void VBrowser::calculate_trace(vid_file obj) {
  ...
  dbCon->save_trace(obj.vdatid, data);  
  return;
}
...
void VBrowser::asc_clicked() {
  ...
  img.set_from_icon_name(iname,Gtk::ICON_SIZE_BUTTON);
  asc_button->set_image(img);
  asc_button->set_use_stock();
  fFBox->invalidate_sort();
}
